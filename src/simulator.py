...
def simulate_stock_transactions(order_book, num_orders):
    ...
    for _ in range(num_orders):
        ...
        quantity = random.randint(1, 1000)  # random quantity between 1 and 1,000
        price = round(random.random()*100, 2)  # random price between 0 and 100

        order_book.add_order(order_type, ticker, quantity, price, display=False)
...
if __name__ == "__main__":
    ...
    run_concurrent_test(order_book, num_threads=10, orders_per_thread=10000)
